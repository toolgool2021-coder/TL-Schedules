...
function initNoteModal() {
    ...
    if (!modal || !noteText) return; // Если элементы не найдены, пропускаем

    function getNoteKey(dayOfWeek, lessonNum) {
        return `lesson_note_${dayOfWeek}_${lessonNum}`;
    }

    window.openNoteModal = function(dayOfWeek, lessonNum, lessonName) {
        currentNoteLesson = { dayOfWeek, lessonNum };
        const noteKey = getNoteKey(dayOfWeek, lessonNum);
        const savedNote = localStorage.getItem(noteKey) || '';
        
        document.getElementById('modalTitle').textContent = `Заметка: ${lessonName}`;
    ...
    function saveNote() {
        ...
        const noteKey = getNoteKey(currentNoteLesson.dayOfWeek, currentNoteLesson.lessonNum);
...
// ===== ФУНКЦИИ ДЛЯ ��АСПИСАНИЯ И ТАЙМЕРА =====
...
function updateTodaySchedule() {
    ...
    for (let i = 0; i < todayLessons.length; i++) {
        ...
        if (hasNote) {
            scheduleItem.classList.add('has-note');
        }
        ...
        scheduleItem.querySelector('.note-icon').addEventListener('click', (e) => {
            e.stopPropagation();
