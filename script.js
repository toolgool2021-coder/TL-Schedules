...
function openNoteModal(dayOfWeek, lessonNum, lessonName) {
    ...
    document.getElementById('modalTitle').textContent = `Заметка: Урок ${lessonNum} - ${lessonName}`;
...
function getCurrentLessonInfo() {
    ...
    // На выходных уроков нет
    if (isWeekend()) {
        return { hasLesson: false, currentLesson: null, nextLesson: null, allLessonsFinished: true, isWeekend: true };
    }
    ...
    return { hasLesson: !!currentLesson, currentLesson, nextLesson, allLessonsFinished, currentMinutes, isWeekend: false };
}
...
function updateDisplay() {
    ...
    if (lessonInfo.isWeekend) {
        document.getElementById('currentLesson').textContent = '🎉 Выходной день!';
        document.getElementById('timerValue').textContent = '🎉';
        statusEl.textContent = '🌟 Время отдыхать!';
        if (!statusEl.querySelector('.weekend-indicator')) {
            const indicator = document.createElement('p');
            indicator.className = 'weekend-indicator';
            indicator.textContent = '✨ Суббота и Воскресенье - выходные дни';
...
function updateTodaySchedule() {
    ...
    for (let i = 0; i < todayLessons.length; i++) {
        ...
        scheduleItem.innerHTML = `
            ...
            <button class="note-btn ${hasNote ? 'has-note' : ''}">📝</button>
        `;

        scheduleItem.querySelector('.note-btn').addEventListener('click', (e) => {
            e.stopPropagation();
