...
function updateDisplay() {
    ...
    if (dayOfWeek === 5 || dayOfWeek === 6) {
        ...
        const timeLeftMs = (endTimeMinutes - lessonInfo.currentMinutes) * 60 * 1000;
        ...
        const timeLeftMs = (startTimeMinutes - lessonInfo.currentMinutes) * 60 * 1000;
        updateTimer(timeLeftMs);
...
function updateTimer(milliseconds) {
    ...
    document.getElementById('timerMs').textContent = String(ms).padStart(3, '0');
}
...
// ===== ОБНОВЛЯЕМ ДИСПЛЕЙ ПОСТОЯННО (для миллисекунд) =====
updateDisplay();
