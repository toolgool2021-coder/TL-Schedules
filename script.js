...
function openNoteModal(dayOfWeek, lessonNum, lessonName) {
    ...
    const noteKey = 'lesson_note_' + dayOfWeek + '_' + lessonNum;
    const savedNote = localStorage.getItem(noteKey) || '';
    
    modalTitle.textContent = 'Заметка: ' + lessonName;
    noteText.value = savedNote;
    modal.style.display = 'block';
    noteText.focus();
    
    window.currentNoteData = { dayOfWeek, lessonNum, noteKey };
}

function closeNoteModal() {
    const modal = document.getElementById('noteModal');
    if (modal) modal.style.display = 'none';
    window.currentNoteData = null;
}

function saveNote() {
    const noteText = document.getElementById('noteText');
    if (!window.currentNoteData || !noteText) return;
    
    const note = noteText.value.trim();
    if (note) {
        localStorage.setItem(window.currentNoteData.noteKey, note);
    } else {
        localStorage.removeItem(window.currentNoteData.noteKey);
    }
    closeNoteModal();
    updateTodaySchedule();
}
...
function deleteNote() {
    if (!window.currentNoteData) return;
    localStorage.removeItem(window.currentNoteData.noteKey);
    closeNoteModal();
    updateTodaySchedule();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (closeX) closeX.onclick = closeNoteModal;
...
function updateTodaySchedule() {
    ...
    for (let i = 0; i < todayLessons.length; i++) {
        ...
        const currentMinutes = now.getHours() * 60 + now.getMinutes();
        const noteKey = 'lesson_note_' + dayOfWeek + '_' + lesson.num;
        const hasNote = localStorage.getItem(noteKey) !== null;
        
        const scheduleItem = document.createElement('div');
        scheduleItem.className = 'schedule-item';
        if (hasNote) scheduleItem.classList.add('has-note');
        if (currentMinutes >= lesson.startMinutes && currentMinutes <= lesson.endMinutes) {
        ...
        scheduleItem.innerHTML = '<div class="lesson-number">Урок ' + lesson.num + '</div>' +
                                 '<div class="lesson-name">' + lessonName + '</div>' +
                                 '<div class="lesson-time">' + lesson.start + ' - ' + lesson.end + '</div>' +
                                 '<div class="note-icon" title="Добавить заметку">📝</div>';

        scheduleItem.querySelector('.note-icon').addEventListener('click', function(e) {
            e.stopPropagation();
    ...
    for (let day = 0; day < 5; day++) {
        ...
        if (dayInfo.lessons.length === 0) {
            ...
            for (let i = 0; i < dayInfo.lessons.length; i++) {
                ...
                lessonsHtml += '<div class="day-lesson">' +
        ...
        dayDiv.innerHTML = '<div class="day-name">' + dayInfo.name + '</div>' +
